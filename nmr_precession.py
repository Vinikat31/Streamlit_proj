...
# --- Inputs do usuário ---
st.sidebar.header("Simulação NMR")
...
def update_graph(theta, phi, omega_ref, wnut, phi_p, tau, time):
    ...
    # Trajectory
    fig.add_trace(go.Scatter3d(x=xlist, y=ylist, z=zlist, mode='lines', line=dict(color='gray', width=2)))
        ...
        updatemenus=[
            dict(
                ...
                buttons=[
                    dict(label="Play",
                         method="animate",
                         args=[None, dict(frame=dict(duration=time, redraw=True),
                                          fromcurrent=True)]),
                    dict(label="Pause",
                         method="animate",
...
fig = update_graph(theta, phi, omega_ref, wnut, phi_p, tau, time_relax)
